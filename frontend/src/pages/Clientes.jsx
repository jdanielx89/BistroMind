...
const Clientes = () => {
  ...
  const abrirDetalle = async (cliente) => {
    ...
    setHistorial([]); // Limpiar historial anterior mientras carga el nuevo
    setCargandoHistorial(true);
    try {
      const res = await fetch(`${API_BASE}/${cliente.telefono}/historial`);
      const data = await res.json();
      // Guardamos la data asegurándonos de que sea un Array
          ...
          {filtrados.map(cli => (
            ...
            >
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-black/40 rounded-2xl flex items-center justify-center text-[#00b4d8] font-black border border-white/5 group-hover:bg-[#00b4d8] group-hover:text-black transition-all italic text-xl">
                ...
                <div>
                  ...
                  <span className="text-[8px] bg-[#00b4d8]/10 text-[#00b4d8] px-2 py-0.5 rounded-full font-black uppercase mt-1 inline-block tracking-tighter">
      ...
      {clienteSeleccionado && (
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative w-full md:max-w-md bg-[#0f171e] h-full shadow-2xl p-6 md:p-10 overflow-y-auto border-l border-[#00b4d8]/20 animate-in slide-in-from-right duration-300">
            ...
            <button onClick={() => setClienteSeleccionado(null)} className="absolute top-6 right-6 text-gray-500 hover:text-white transition-colors">
              <X size={24}/>
            </button>

            <div className="mt-8">
              <div className="w-20 h-20 bg-[#00b4d8] rounded-[2rem] flex items-center justify-center text-black text-4xl font-black mb-6 italic shadow-xl shadow-[#00b4d8]/10">
                {clienteSeleccionado.nombre.charAt(0).toUpperCase()}
              </div>
              <h2 className="text-3xl font-black italic uppercase leading-none tracking-tighter">{clienteSeleccionado.nombre}</h2>
              ...
              <div className="bg-[#1b262f] p-6 rounded-2xl border border-white/5 mb-8">
                <p className="text-[8px] text-gray-500 font-black uppercase tracking-widest mb-2">Notas de Preferencia</p>
                <p className="text-sm italic text-gray-300 leading-relaxed">
                  {clienteSeleccionado.notas_frecuentes ? `"${clienteSeleccionado.notas_frecuentes}"` : "Sin requerimientos especiales."}
                </p>
              </div>

              <h3 className="text-[10px] font-black uppercase text-gray-500 mb-4 flex items-center gap-2 tracking-[0.2em]">
                <Calendar size={14} className="text-[#00b4d8]"/> Historial de Visitas
              </h3>

              <div className="space-y-3">
                {cargandoHistorial ? (
                  <div className="text-center py-10 text-[#00b4d8] font-black uppercase text-[10px] animate-pulse">Cargando registros...</div>
                ) : historial.length > 0 ? (
                  historial.map((res, i) => (
                    <div key={i} className="bg-[#1b262f] p-4 rounded-xl border border-white/5 shadow-md">
                      <div className="flex justify-between font-black text-sm italic mb-2">
                        <span className="text-white">
                          {res.fecha ? new Date(res.fecha).toLocaleDateString('es-ES', { day: '2-digit', month: 'short', year: 'numeric' }) : '---'}
                        </span>
                        <span className="text-[#00b4d8]">{res.hora} HS</span>
                      </div>
                      <div className="flex gap-4 text-[9px] text-gray-500 font-black uppercase">
                        <span className="flex items-center gap-1.5"><Users size={12}/> {res.num_personas} Pax</span>
                        <span className="flex items-center gap-1.5"><Star size={12}/> {res.mesa || 'S/A'}</span>
                        <span className={`ml-auto ${res.estado === 'Llego' ? 'text-cyan-400' : 'text-gray-600'}`}>
                          {res.estado === 'Llego' ? '● COMPLETADA' : `● ${res.estado}`}
                        </span>
                      </div>
                    </div>
                  ))
                ) : (
                  <div className="text-gray-700 text-center py-10 font-bold italic uppercase text-xs border-2 border-dashed border-white/5 rounded-2xl">
